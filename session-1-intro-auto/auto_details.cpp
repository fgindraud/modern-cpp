...
#ifdef AUTO_INIT
std::vector<int> makeIntVector() { return {1, 2, 3, 4}; }
...
int main() {
    ...
    auto vec2 = makeIntVector();
    // instead of
    std::vector<int> vec3 = makeIntVector();
...
void print_front(const Container& container) {
...
int main() {
    f(42);
    print_front(std::vector<int>{1, 2, 3});
}
#endif
...
auto front(const std::vector<int>& v) { return v.front(); }

// Force reference return:
auto& front_ref(const std::vector<int>& v) { return v.front(); }

// Ask for "raw" type deduction, will return exactly the same expression type:
decltype(auto) front_raw(const std::vector<int>& v) { return v.front(); }
...
auto front(const std::vector<int>& v) -> decltype(v.front()) { return v.front(); }
