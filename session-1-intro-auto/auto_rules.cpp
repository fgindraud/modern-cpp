...
#include <type_traits> // tools to compare types, not explained

// Create an int type which prints when copied
struct NoisyInt {
	int v;
...
// This function is used to pretty print the name of type T.
// Just ignore its implementation, it will not be detailed
// (T, name) -> "name: T"
template <typename T> void show_type (const std::string name) {
	std::cout << name << ": ";
